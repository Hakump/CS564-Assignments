...
void intTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	// NEW here
    checkPassFail(intScan(&index, 4500, GTE, 6000, LTE), 501)
...
void test4(){
    relationSize = 600000;
    // in case of 600000, the root will not split forward, but will backward, expected random
    std::cout<<"test4-large input start: \n";
    createRelationForward();
    indexTests();
    deleteRelation();

    std::cout<<"test4-large input 2 start: \n";

    createRelationBackward();
    indexTests();
    deleteRelation();

    std::cout<<"test4-large input 3 start: \n";
