 ...
 public:
  ...
  /**
	...
	**/
    ...
    struct NewNode insertHelper(PageId pageId, RIDKeyPair<int> ridKeyPair, int level);

    struct NewNode leafSpliter(int position, LeafNodeInt *fullLeaf, RIDKeyPair<int> ridKeyPair);

    struct NewNode nonLeafSpliter(int position, NonLeafNodeInt *fullLeaf, NewNode &newnode);
};
